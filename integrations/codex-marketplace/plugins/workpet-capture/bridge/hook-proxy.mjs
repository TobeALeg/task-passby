...
try {
  ...
  if (config.host !== "127.0.0.1") throw new Error("WorkPet bridge 只能使用 loopback");
  ...
  process.stderr.write(`WorkPet Codex hook 未同步：${error instanceof Error ? error.message : String(error)}\n`);
  process.stdout.write(JSON.stringify({ continue: true }));
}
