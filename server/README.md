...
桌面端每次沉淀与创建复用工作分别勾选“参与改进 Worket”；默认不勾选，历史记录不会因升级自动上传。保存期限在 `src/contracts/improvement.ts` 定义为授权起 90 天。使用者从“更多 → Worket 服务 → 改进数据”停止和删除。采集不会增加模型调用，模型未配置也可独立接收已授权样本。
