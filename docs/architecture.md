...
### 产品改进数据链（2026-09-09 新目标，待实现）

当前 `Worket AI Service` 的输入和候选仅暂存内存。新目标将增加独立的改进样本存储与反馈接口：客户端确认范围和改进用途授权 → 提交固定材料 → 关联模型候选及版本 → 追加用户实际修订和确认 → 经本次范围授权关联复用反馈 → 管理员评审并构建效果评测。
...
样本接口复用服务端验证后的用户主体，增加样本/事件标识以保证重传幂等、来源追溯与主体隔离；管理员查看与客户端提交权限分开。删除必须传递到含正文的派生评测副本。日志和 Provider 的能力保持原边界。具体采集字段及待定留存参数见 Spec 9.1.1；本节描述目标结构，尚未对应已启用代码。
