...
本轮交付范围：后台查看/评审/删除、暂停接收；客户端每次范围独立勾选、离线队列、自动反馈、停止和删除。复用首版只保存非文件输入和逐项验收，以及交付 ID；不采集新对话或交付文件正文。沉淀重试默认不采集改进样本。旧 `worket-data-v1` 仍只允许模型处理，自动升级授权被拒绝。本机已有人工整理样本未导入，未调用 DeepSeek。证据见 [改进采集验收](../acceptance/worket-improvement.md)。
