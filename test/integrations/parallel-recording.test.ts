...
function fixture() {
  ...
  const service = new AppService({ databasePath: ":memory:", codex,
...
test("并行活动导致前台不唯一时，面板来源仍列出两项，沉睡聊天可以翻页加入", async () => {
  ...
  try {
    ...
    const sources = await service.listCodexThreads();
    ...
    for (const source of sources) await service.createWorkFromCodex({ threadId: source.id });
    const firstPage = await service.listCodexHistory();
    assert.ok(firstPage.threads.every((t) => t.workId));
    const older = await service.listCodexHistory(firstPage.nextCursor!);
    assert.equal(older.threads[0]?.title, "沉睡 C");
    await service.createWorkFromCodex({ threadId: older.threads[0]!.id });
    const repeated = await service.createWorkFromCodex({ threadId: "0" });
...
test("未收到 Hook 时所有已选聊天仍增量同步，未选聊天不读取，单项失败不阻塞其他工作", async () => {
  ...
  try {
    await service.createWorkFromCodex({ threadId: "0" });
    await service.createWorkFromCodex({ threadId: "1" });
...
test("会话读取期间用户完成工作，返回的数据不再写入", async () => {
  ...
  try {
    const created = await service.createWorkFromCodex({ threadId: "0" });
    ...
    assert.equal((await service.syncCodexHook({ session_id: "0" })).accepted, false);
