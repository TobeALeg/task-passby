...
- 状态：原开发闭环已实现并通过合成验证，真实模型连接已验证；固定集质量、真实第二次工作与公开分发仍需验收。2026-09-09 新增的样本库与单次授权采集已实现并通过合成验证；真实样本导入与改进效果对比未执行。证据及差距见 [验收记录](../acceptance/work-distillation-v1.md)。
...
D13 是 2026-09-09 新确认并实现的产品方向；样本库与授权采集已通过合成验证。原 V1 的“不留正文”只适用于未获改进授权的任务；真实样本导入与改进效果评测仍待执行。
...
当前代码的处理模式：分析任务材料保留在内存，候选结果默认最多暂存 10 分钟，客户端确认收到或用户取消后清除。独立改进授权启用后，任务结果缓存与经授权的改进样本分别管理：ack 只代表客户端收到候选，不再等同于删除改进样本。新模式启用前必须同步更新客户端说明和授权版本；旧版仅同意模型处理的授权，不能自动升级为允许长期留存和产品改进。运行日志、异常上报和代理转储仍不得记录正文。
...
停止后不再上传新的改进样本；删除请求要覆盖原材料、候选、修改差异及包含正文的派生评测副本。保留期限集中定义为自授权时间起 **90 天**，后续反馈不续期。到期在运行中的服务定时清理、请求或重启时删除。当前不另存含正文的派生评测副本；评审备注与样本一并删除。

新增采集链的验收要求：未授权零正文留存；授权与样本对应；同一事件重传不重复计数；用户修改与模型原稿可区分；复用使用自己的范围授权；主体隔离和删除闭环通过；能够从一个真实纠正样本构造评测并比较后续版本。当前已完成采集与管理的合成验收；最后一项真实样本效果对比尚未执行，不能宣称改进效果已验证。

本轮交付范围：后台查看/评审/删除、暂停接收；客户端每次展示范围与采集选项、离线队列、自动反馈、停止和删除。2026-09-10 用户明确改为默认勾选、直到手动取消；该偏好保存在本地工作库，沉淀和复用共用，取消即停止全部样本的后续采集，重启不重置。重新开启仅用于此后提交的范围，不恢复旧订阅；该交互变更暂未打包发布。复用首版只保存非文件输入和逐项验收，以及交付 ID；不采集新对话或交付文件正文。沉淀重试默认不采集改进样本。旧 `worket-data-v1` 仍只允许模型处理，自动升级授权被拒绝。本机已有人工整理样本未导入，未调用 DeepSeek。证据见 [改进采集验收](../acceptance/worket-improvement.md)。
