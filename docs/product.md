...
桌宠圆点表达全部工作的采集状态：至少一项真实绑定正在记录时为绿色；只有待绑定工作时为黄色；没有有效采集时为灰色。该状态在重启后从本地记录恢复，不随选中工作或前台应用变化。气泡仍仅提示当前识别到的会话，待绑定时显示“等待绑定”。面板的“进行中”是工作生命周期分组，组内分别显示“正在记录”“等待绑定”“已停止记录”。多项 Codex 工作可各自记录，WorkBuddy 无标题模式仍限制为单项活动记录。
